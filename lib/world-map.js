...
jvm.WorldMap = function(params) {
  ...
  this.createRegions();
  if(this.params.showConnector) {
	this.createConnector(this.params.connectors || {});
  }
  this.createMarkers(this.params.markers || {});
  if(this.params.showCaption) {
	this.createCaptions(this.params.markers || {});
  }
  ...
  if (this.params.selectedConnectors) {
	this.setSelectedConnectors(this.params.selectedConnectors);
  }
...
jvm.WorldMap.prototype = {
  ...
  bindElementEvents: function(){
	...
	// MODIFIED CODE
		var path = this,
		baseVal = jvm.$(this).attr('class').baseVal ? jvm.$(this).attr('class').baseVal : jvm.$(this).attr('class'),
		type = baseVal.indexOf('jvectormap-region') === -1 ? (baseVal.indexOf('jvectormap-marker') === -1 ? (baseVal.indexOf('jvectormap-connector') === -1 ? 'caption' : 'connector') : 'marker' ) : 'region',
		code = type == 'region' ? jvm.$(this).attr('data-code') : jvm.$(this).attr('data-index'),
		element = type == 'region' ? map.regions[code].element : ((type == 'marker' || type == 'caption') ? map.markers[code].element : map.connectors[code].element),
		labelText = type == 'region' ? map.mapData.paths[code].name : ((type == 'marker' || type == 'caption') ? (map.markers[code].config.name || '') : (map.connectors[code].pLineDetails.routeOrg + " - " + map.connectors[code].pLineDetails.routeDstn)),
		labelShowEvent = jvm.$.Event(type+'LabelShow.jvectormap'),
		overEvent = jvm.$.Event(type+'Over.jvectormap');
    ...
    this.container.delegate("[class~='jvectormap-element']", 'mouseup', function(e){
      var path = this,
          baseVal = jvm.$(this).attr('class').baseVal ? jvm.$(this).attr('class').baseVal : jvm.$(this).attr('class'),
          type = baseVal.indexOf('jvectormap-region') === -1 ? (baseVal.indexOf('jvectormap-marker') === -1 ? 'connector' : 'marker') :  'region',
          code = type == 'region' ? jvm.$(this).attr('data-code') : jvm.$(this).attr('data-index'),
          clickEvent = jvm.$.Event(type+'Click.jvectormap'),
          element = type == 'region' ? map.regions[code].element : (type == 'marker' ? map.markers[code].element : map.connectors[code].element);

      if (!mouseMoved) {
        map.container.trigger(clickEvent, [code]);
        if ((type === 'region' && map.params.regionsSelectable) || (type === 'marker' && map.params.markersSelectable) || (type === 'connector' && map.params.connectorsSelectable) ) {
  ...
  /**
   * Return the codes of currently selected connectors.
   * @returns {Array}
   */
  getSelectedConnectors: function(){
    return this.getSelected('connectors');
  },
   ...
   * @param {String|Array|Object} keys If <code>String</code> or <code>Array</code> the region(s) with the corresponding code(s) will be selected. If <code>Object</code> was provided its keys are  codes of regions, state of which should be changed. Selected state will be set if value is true, removed otherwise.
   */
  setSelectedConnectors: function(keys){
    this.setSelected('connectors', keys);
  },
  ...
  //Custom Function to Create the connecter between maker
  createConnector: function(connectors){
    var i,
        connector,
        point,
		pathStr,
    ...
    //this.connectorGroup = this.connectorGroup || this.canvas.addGroup();
	this.markersGroup = this.markersGroup || this.canvas.addGroup();

    if (jvm.$.isArray(connectors)) {
      connectorArray = connectors.slice();
      connectors = {};
      for (i = 0; i < connectorArray.length; i++) {
        connectors[i] = connectorArray[i];
      }
    }

    for (i in connectors) {
	  connectorConfig = connectors[i] instanceof Array ? {latLng: connectors[i]} : connectors[i];
	  point = this.getConnectorPosition( connectorConfig );
  
      connector = this.canvas.addPath({
        d: this.convertArrayToPath(point),
        "data-index": i
      }, jvm.$.extend(true, {}, this.params.connectorStyle, {initial: connectorConfig.style || {}}), this.markersGroup);
	  
      jvm.$(connector.node).bind('selected', function(e, isSelected){
        map.container.trigger('connectorSelected.jvectormap', [jvm.$(this).attr('data-code'), isSelected, map.getSelectedConnectors()]);
      });
	  
      connector.addClass('jvectormap-connector jvectormap-element');
	  
	  if (this.connectors[i]) {
        this.removeConnectors([i]);
      }
	
	  this.connectors[i] = {
        element : connector,
		latLang : { latLngFrom : connectors[i].latLngFrom, latLngTo : connectors[i].latLngTo },
		pLineDetails: connectors[i].pLineDetails || {}
      };
    }
	
	$(".jvectormap-connector").each(function(idx, item) {
		$(this).attr({ "id" : "connector-"+idx });
		
		var details = connectors[idx].pLineDetails;
		
		var str = '<div class="pline-details">';
		str += '<table id="tblPipeLine" border="0" cellpadding="0" cellspacing="0">';
		str += '<tr><th>Routing</th><td id="tdRoute">' + details.routeOrg + ' - ' + details.routeDstn + '</td>';
		str += '</tr><tr><th>Movement</th><td id="tdMovement">'+details.type+'</td></tr>';
		str += '<tr><th>Product</th><td id="tdProduct">'+details.product+'</td></tr>';
		str += '<tr><th>Contract</th><td id="tdContract">'+details.contract+'</td></tr>';
		str += '</table></div>';
		
		Tipped.create("#connector-"+idx, str, {
			skin: 'white',
			hook: 'topleft',
			hideOn: false,
			closeButton: true,
  ...
  createMarkers: function(markers) {
	...
	//Assigning Click Event for the Markers
	$(".jvectormap-marker").each(function(idx, item) {
		$(this).attr({ "id" : "marker-"+idx });
		
		Tipped.create("#marker-"+idx, "<b>"+markers[idx].name+"</b>", {
			skin: 'white',
			hook: 'topleft',
			hideOn: false,
			closeButton: true,
  ...
  createCaptions: function(captions) {
    ...
    this.captionsGroup = this.captionsGroup || this.canvas.addGroup();
	
    if (jvm.$.isArray(captions)) {
      captionsArray = captions.slice();
      captions = {};
      for (i = 0; i < captionsArray.length; i++) {
        captions[i] = captionsArray[i];
      }
    }

    for (i in captions) {
 
	  captionConfig = captions[i] instanceof Array ? {latLng: captions[i]} : captions[i];
	  point = this.getCaptionPosition( captionConfig );
	  
	  textConfig = {
		caption: markers[i].name,
		latLang: markers[i].latLang,
	  };
	  
      if (point !== false) {
		caption = this.canvas.addText({
          "data-index": i,
		  caption: markers[i].name,
          x: point.x,
          y: point.y
        }, jvm.$.extend(true, {}, this.params.captionStyle, {initial: textConfig.style || {}}), this.markersGroup);
		
        caption.addClass('jvectormap-caption jvectormap-element');
        if (this.captions[i]) {
          this.removeCaptions([i]);
        }
        this.captions[i] = {element: caption, config: captionConfig};
      }
    }
  ...
  repositionCaptions: function() {
    var i,
        point;

    for (i in this.captions) {
      point = this.getCaptionPosition( this.captions[i].config );
      if (point !== false) {
        this.captions[i].element.setStyle({x: point.x, y: point.y});
      }
    }
	
  },
  
  repositionConnectors: function() {
    var i, point, pathStr, that = this;

    for (i in this.connectors) {
      point 	= this.getConnectorPosition( this.connectors[i].latLang );
	  pathStr	= that.convertArrayToPath( point );
	  
      if (point !== false) {
        this.connectors[i].element.setStyle({d: pathStr});
      }
    }
  ...
  getCaptionPosition: function(captionConfig) {
    if (jvm.WorldMap.maps[this.params.map].projection) {
      return this.latLngToPoint.apply(this, captionConfig.latLng || [0, 0]);
    } else {
      return {
        x: captionConfig.coords[0]*this.scale + this.transX*this.scale,
        y: captionConfig.coords[1]*this.scale + this.transY*this.scale
      };
    }
  },
  
  getConnectorPosition: function(connectorConfig) {
    if (jvm.WorldMap.maps[this.params.map].projection) {
      return [this.latLngToPoint.apply(this, connectorConfig.latLngFrom || [0, 0]), this.latLngToPoint.apply(this, connectorConfig.latLngTo || [0, 0])];
    } else {
      return {
        x: connectorConfig.coords[0]*this.scale + this.transX*this.scale,
        y: connectorConfig.coords[1]*this.scale + this.transY*this.scale
      };
    }
  },
  
  //Custom Modification
  convertArrayToPath: function(arrVal) {
	var arr = "M", xPos, yPos;
	xPos = -(arrVal[0].x - arrVal[1].x);
	yPos = -(arrVal[0].y - arrVal[1].y);
	
	arr += (arrVal[0].x).toFixed(2) + "," + (arrVal[0].y).toFixed(2) + "l";
	arr += xPos.toFixed(2) + "," + yPos.toFixed(2) + "Z";
  ...
  /**
   * Remove some Connectors from the map.
   * @param {Array} connectors Array of connector codes to be removed.
   */
  removeConnectors: function(connectors){
    var i;

    for (i = 0; i < connectors.length; i++) {
      this.connectors[ connectors[i] ].element.remove();
      delete this.connectors[ connectors[i] ];
    };
  },
...
jvm.WorldMap.apiEvents = {
  ...
  onConnectorLabelShow: 'connectorLabelShow',
  onConnectorOver: 'connectorOver',
  onConnectorOut: 'connectorOut',
  onConnectorClick: 'connectorClick',
